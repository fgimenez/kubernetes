...
By default, the Vagrant setup will create a single master VM (called kubernetes-master) and one node (called kubernetes-node-1). The master VM will take ~1.6 GB and each node 1 GB, so make sure you have at least 2.6GB to 4.6GB of free memory (plus appropriate free disk space).
